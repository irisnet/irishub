...
import (
	...
	"github.com/irisnet/irishub/migrate/privval"
)
...
func TestLoadAndUpgrade(t *testing.T) {
	...
	assert.NoError(t, err)
	upgradedPV := tmprivval.LoadFilePV(newKeyFile.Name(), newStateFile.Name())
...
func assertEqualPV(t *testing.T, oldPV *privval.OldFilePV, newPV *tmprivval.FilePV) {
	...
	pubKey, _ := newPV.GetPubKey()
	assert.Equal(t, oldPV.PubKey, pubKey)
	assert.Equal(t, oldPV.PrivKey, newPV.Key.PrivKey)

	assert.Equal(t, oldPV.LastHeight, newPV.LastSignState.Height)
	assert.Equal(t, oldPV.LastRound, int(newPV.LastSignState.Round))
