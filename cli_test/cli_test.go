...
func TestIrisCLIKeysAddRecover(t *testing.T) {
	...
	require.Equal(t, "iaa1qcfdf69js922qrdr4yaww3ax7gjml6pdcj4tty", f.KeyAddress("test-recover").String())
...
func TestIrisCLIKeysAddRecoverHDPath(t *testing.T) {
	...
	require.Equal(t, "iaa1qcfdf69js922qrdr4yaww3ax7gjml6pdcj4tty", f.KeyAddress("test-recoverHD1").String())
	...
	require.Equal(t, "iaa1pdfav2cjhry9k79nu6r8kgknnjtq6a7r35mvt4", f.KeyAddress("test-recoverH2").String())
	...
	require.Equal(t, "iaa1909k354n6wl8ujzu6kmh49w4d02ax7qv25vyjz", f.KeyAddress("test-recoverH3").String())
	...
	require.Equal(t, "iaa1v9plmhvyhgxk3th9ydacm7j4z357s3nh7vsrl6", f.KeyAddress("test-recoverH4").String())
...
func TestIrisCLIQueryRewards(t *testing.T) {
	...
	f.cdc.MustUnmarshalJSON(genesisState[mint.ModuleName], &mintData)
...
`, proposalTokens.String())
	...
	// ensure deposit was deducted
	require.Equal(t, startTokens.Sub(proposalTokens).String(), f.QueryBalances(fooAddr).AmountOf(denom).String())
...
func TestIrisCLISendGenerateSignAndBroadcast(t *testing.T) {
	...
	require.Equal(t, startTokens, f.QueryBalances(barAddr).AmountOf(denom))
	require.Equal(t, startTokens.Sub(sendTokens), f.QueryBalances(fooAddr).AmountOf(denom))
...
func TestIrisCLIEncode(t *testing.T) {
	...
	decodedTx := cli.UnmarshalStdTx(t, f.cdc, trimmedBase64)
...
func TestIrisCLIConfig(t *testing.T) {
	...
	config, err := ioutil.ReadFile(path.Join(f.IrisHome, "config", "config.toml"))
	require.NoError(t, err)
