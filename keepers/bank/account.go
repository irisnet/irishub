...
// nolint
func NewAccountKeeper(cdc *codec.Codec, key sdk.StoreKey, proto func() bank.Account) AccountKeeper {
...
func (am AccountKeeper) NewAccountWithAddress(ctx sdk.Context, addr sdk.AccAddress) bank.Account {
	acc := am.proto()
...
// New Account
func (am AccountKeeper) NewAccount(ctx sdk.Context, acc bank.Account) bank.Account {
	err := acc.SetAccountNumber(am.GetNextAccountNumber(ctx))
	if err != nil {
...
func (am AccountKeeper) GetAccount(ctx sdk.Context, addr sdk.AccAddress) bank.Account {
...
// Implements sdk.AccountKeeper.
func (am AccountKeeper) SetAccount(ctx sdk.Context, acc bank.Account) {
	addr := acc.GetAddress()
	store := ctx.KVStore(am.key)
...
func (am AccountKeeper) RemoveAccount(ctx sdk.Context, acc bank.Account) {
	addr := acc.GetAddress()
	store := ctx.KVStore(am.key)
...
// Implements sdk.AccountKeeper.
func (am AccountKeeper) IterateAccounts(ctx sdk.Context, process func(bank.Account) (stop bool)) {
	store := ctx.KVStore(am.key)
...
func (am AccountKeeper) encodeAccount(acc bank.Account) []byte {
...
func (am AccountKeeper) decodeAccount(bz []byte) (acc bank.Account) {
