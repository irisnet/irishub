...
```shell
iriscli service bind --chain-id=test --from=node0 --fee=0.12iris --service-name=test-service --def-chain-id=test --bind-type=Local --deposit=1000iris --prices=1iris --avg-rsp-time=10000 --usable-time=100
```
