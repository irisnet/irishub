...
```shell
iriscli service call --chain-id=<chain-id> --from=node0 --fee=0.3iris --def-chain-id=<chain-id> --service-name=test-service --method-id=1 --bind-chain-id=test --provider=iaa1qm54q9ta97kwqaedz9wzd90cacdsp6mq54cwda --service-fee=1iris --request-data=434355
```
