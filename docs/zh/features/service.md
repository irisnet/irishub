...
```
# 创建服务定义
iriscli service define --chain-id=service-test  --from=x --fee=0.12iris --service-name=test-service --service-description=service-description --author-description=author-description --tags=tag1,tag2 --idl-content=<idl-content> --file=test.proto
...
```
# 服务绑定
iriscli service bind --chain-id=service-test  --from=x --fee=0.12iris --service-name=test-service --def-chain-id=service-test --bind-type=Local  --deposit=1000iris --prices=1iris --avg-rsp-time=10000 --usable-time=100
...
# 服务绑定更新
iriscli service update-binding --chain-id=service-test  --from=x --fee=0.12iris --service-name=test-service --def-chain-id=service-test --bind-type=Local  --deposit=1iris --prices=1iris,2iris --avg-rsp-time=10000 --usable-time=100

# 禁用服务绑定
iriscli service disable --chain-id=service-test  --from=x --fee=0.12iris --def-chain-id=service-test --service-name=test-service

# 开启服务绑定
iriscli service enable --chain-id=service-test  --from=x --fee=0.12iris --def-chain-id=service-test --service-name=test-service --deposit=1iris

# 取回押金
iriscli service refund-deposit --chain-id=service-test  --from=x --fee=0.12iris --def-chain-id=service-test --service-name=test-service
```
...
# 响应服务调用
iriscli service respond --chain-id=test --from=node0 --fee=0.12iris --request-chain-id=test --request-id=230-130-0 --response-data=abcd
...
# 从服务费退款中退还所有费用
iriscli service refund-fees --chain-id=test --from=node0 --fee=0.12iris
