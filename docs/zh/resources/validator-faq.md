...
-   **不在线**： 验证节点长期不参与网络共识，即如果假设每5秒产生一个新的区块，在24小时内将产生34,560个块。在一天中，如果一个验证节点的签名区块数量少于34,560的 30%，则对应的验证人抵押0.03%的通证（包括委托人委托的通证）将被罚没。该验证节点将会被处罚不能参加共识1.5天，这样也无法获得抵押获利。
