...
func (k Keeper) IssueToken(ctx sdk.Context, token types.FungibleToken) sdk.Error {
	if token.GetSource() == types.GATEWAY {
		...
		if !gateway.Owner.Equals(token.GetOwner()) {
			return types.ErrUnauthorizedIssueGatewayAsset(k.codespace,
	...
	if owner != nil {
		...
		_, err := k.bk.AddCoins(ctx, owner, sdk.Coins{initialSupply})
		if err != nil {
			return err
		}
	}
...
func (k Keeper) CreateGateway(ctx sdk.Context, msg types.MsgCreateGateway) sdk.Error {
	// check if the moniker already exists
	if k.HasGateway(ctx, msg.Moniker) {
		return types.ErrGatewayAlreadyExists(k.codespace, fmt.Sprintf("the moniker already exists:%s", msg.Moniker))
	}
...
func (k Keeper) EditGateway(ctx sdk.Context, msg types.MsgEditGateway) sdk.Error {
	...
	if !msg.Owner.Equals(gateway.Owner) {
		return types.ErrInvalidOwner(k.codespace, fmt.Sprintf("the address %d is not the owner of the gateway %s", msg.Owner, msg.Moniker))
	}
...
func (k Keeper) EditToken(ctx sdk.Context, msg types.MsgEditToken) sdk.Error {
	...
	if !found {
		return types.ErrAssetNotExists(k.codespace, fmt.Sprintf("token denom %s does not exist", token.GetDenom()))
	}
	...
	if maxSupply.GT(sdk.ZeroInt()) && (maxSupply.LT(hasIssuedAmt.Amount) || maxSupply.GT(token.MaxSupply)) {
		return types.ErrInvalidAssetMaxSupply(k.codespace, fmt.Sprintf("max supply must not be less than %s and greater than %s", hasIssuedAmt.Amount.String(), token.MaxSupply.String()))
	}
...
func (k Keeper) TransferGatewayOwner(ctx sdk.Context, msg types.MsgTransferGatewayOwner) sdk.Error {
	// get the destination gateway
	gateway, err := k.GetGateway(ctx, msg.Moniker)
	if err != nil {
		return err
...
func (k Keeper) MintToken(ctx sdk.Context, msg types.MsgMintToken) sdk.Error {
	...
	if expDenom != hasIssuedAmt.Denom {
		return types.ErrAssetNotExists(k.codespace, fmt.Sprintf("denom of mint token is not equal issued token,expected:%s,actual:%s", expDenom, hasIssuedAmt.Denom))
	}
	...
	if mintAmt.Add(hasIssuedAmt.Amount).GT(token.MaxSupply) {
		...
		canAmt := token.MaxSupply.Sub(hasIssuedAmt.Amount).Quo(exp)
	...
	_, err := k.bk.AddCoins(ctx, mintAcc, sdk.Coins{mintCoin})
	if err != nil {
		return err
	}
