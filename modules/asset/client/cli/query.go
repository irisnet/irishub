...
func GetCmdQueryToken(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", asset.QuerierRoute, asset.QueryToken), bz)
			if err != nil {
				return err
			}
...
func GetCmdQueryTokens(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s", asset.QuerierRoute, asset.QueryTokens), bz)
			if err != nil {
				return err
			}
...
func GetCmdQueryGateway(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/gateway", asset.QuerierRoute), bz)
			if err != nil {
				return err
			}
...
func GetCmdQueryGateways(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/gateways", asset.QuerierRoute), bz)
			if err != nil {
				return err
			}
