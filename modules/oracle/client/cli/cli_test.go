...
func (s *IntegrationTestSuite) TestOracle() {
	...
	args := []string{
		...
		fmt.Sprintf("--%s=%d", oraclecli.FlagDescription, description),
		fmt.Sprintf("--%s=%d", oraclecli.FlagServiceFeeCap, serviceFeeCap),
		fmt.Sprintf("--%s=%t", oraclecli.FlagServiceName, serviceName),
		fmt.Sprintf("--%s=%t", oraclecli.FlagInput, input),
		fmt.Sprintf("--%s=%t", oraclecli.FlagProviders, providers),
		fmt.Sprintf("--%s=%t", oraclecli.FlagTimeout, timeout),
		fmt.Sprintf("--%s=%t", oraclecli.FlagThreshold, threshold),
		fmt.Sprintf("--%s=%t", oraclecli.FlagFrequency, frequency),
		fmt.Sprintf("--%s=%t", oraclecli.FlagCreator, creator),
