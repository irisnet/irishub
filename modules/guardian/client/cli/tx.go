...
func GetCmdCreateProfiler(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "iriscli tx guardian add-profiler --chain-id=<chain-id> --from=<key-name> --fee=0.3iris " +
...
func GetCmdDeleteProfiler(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "iriscli tx guardian delete-profiler --chain-id=<chain-id> --from=<key-name> --fee=0.3iris " +
...
func GetCmdCreateTrustee(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "iriscli tx guardian add-trustee --chain-id=<chain-id> --from=<key-name> --fee=0.3iris " +
			"--address=<added address> --description=<name>",
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			txBldr := auth.NewTxBuilderFromCLI().WithTxEncoder(utils.GetTxEncoder(cdc))
...
func GetCmdDeleteTrustee(cdc *codec.Codec) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: "iriscli tx guardian delete-trustee --chain-id=<chain-id> --from=<key-name> --fee=0.3iris " +
			"--address=<deleted address>",
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			txBldr := auth.NewTxBuilderFromCLI().WithTxEncoder(utils.GetTxEncoder(cdc))
