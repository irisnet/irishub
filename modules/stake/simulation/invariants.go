...
func SupplyInvariants(ck bank.Keeper, k stake.Keeper,
	...
	return func(ctx sdk.Context) error {
		...
		am.IterateAccounts(ctx, func(acc auth.Account) bool {
			loose = loose.Add(sdk.NewDecFromInt(acc.GetCoins().AmountOf(stakeTypes.StakeDenom)))
			return false
		})
		...
		loose = loose.Add(sdk.NewDecFromInt(f.GetCollectedFees(ctx).AmountOf(stakeTypes.StakeDenom)))

		// add community pool
		loose = loose.Add(feePool.CommunityPool.AmountOf(stakeTypes.StakeDenom))
		...
		d.IterateValidatorDistInfos(ctx,
			func(_ int64, distInfo distribution.ValidatorDistInfo) (stop bool) {
				loose = loose.Add(distInfo.DelPool.AmountOf(stakeTypes.StakeDenom))
				loose = loose.Add(distInfo.ValCommission.AmountOf(stakeTypes.StakeDenom))
				return false
			},
		)
		...
		equivalent := pool.BondedTokens.QuoInt(sdk.NewIntWithDecimal(1, 18))

		// Bonded tokens should equal sum of tokens with bonded validators
		if !equivalent.GTE(bonded) || !bonded.GTE(equivalent.Sub(sdk.NewDec(validatorCount))) {
			return fmt.Errorf("bonded token invariance:\n\tpool.BondedTokens: %v"+
				"\n\tsum of account tokens: %v", equivalent, bonded)
		}
