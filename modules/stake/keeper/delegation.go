...
func (k Keeper) Delegate(ctx sdk.Context, delAddr sdk.AccAddress, bondAmt sdk.Coin,
	...
	if subtractAccount {
		...
		ctx.TagsManager().AddCoinFlow(ctx, delAddr.String(), validator.OperatorAddr.String(), bondAmt.String(), sdk.DelegationFlow, "")
...
func (k Keeper) BeginUnbonding(ctx sdk.Context,
	...
	if ctx.BlockHeight() < version.H001_UNDELEGATE_PATCH {
		...
		if completeNow {
			if !balance.IsZero() {
				ctx.TagsManager().AddCoinFlow(ctx, valAddr.String(), delAddr.String(), balance.String(), sdk.UndelegationFlow, ctx.CoinFlowTrigger())
			}
