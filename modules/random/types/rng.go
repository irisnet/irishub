...
const RandomPrec = 20 // the precision for generated random numbers

// RNG is a random number generator
type RNG interface {
	GetRandom() big.Rat // interface which returns a random number between [0,1)
}
...
// GetRandom implements RNG
func (p PRNG) GetRandom() *big.Rat {
	...
	precision := new(big.Int).Exp(big.NewInt(10), big.NewInt(RandomPrec), nil)

	// Generate a random number between [0,1) with `RandomPrec` precision from seed
