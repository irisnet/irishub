...
func validateBasic(tx banktype.StdTx) (err sdk.Error) {
...
func getSignerAccs(ctx sdk.Context, am bank.AccountKeeper, addrs []sdk.AccAddress) (accs []banktype.Account, res sdk.Result) {
	accs = make([]banktype.Account, len(addrs))
...
func validateAccNumAndSequence(ctx sdk.Context, accs []banktype.Account, sigs []banktype.StdSignature) sdk.Result {
	for i := 0; i < len(accs); i++ {
...
func processSig(ctx sdk.Context,
	acc banktype.Account, sig banktype.StdSignature, signBytes []byte, simulate bool) (updatedAcc banktype.Account, res sdk.Result) {
...
func processPubKey(acc banktype.Account, sig banktype.StdSignature, simulate bool) (crypto.PubKey, sdk.Result) {
...
func deductFees(acc banktype.Account, fee banktype.StdFee) (banktype.Account, sdk.Result) {
	coins := acc.GetCoins()
...
func ensureSufficientMempoolFees(ctx sdk.Context, stdTx banktype.StdTx) sdk.Result {
...
func setGasMeter(simulate bool, ctx sdk.Context, stdTx banktype.StdTx) sdk.Context {
	// set the gas meter
...
func getSignBytesList(chainID string, stdTx banktype.StdTx, stdSigs []banktype.StdSignature) (signatureBytesList [][]byte) {
	signatureBytesList = make([][]byte, len(stdSigs))
	for i := 0; i < len(stdSigs); i++ {
		signatureBytesList[i] = banktype.StdSignBytes(chainID,
