...
`,
	...
	cmd.Flags().
		String(srvflags.AppDBBackend, "", "The type of database for application and snapshots databases")

	cmd.Flags().
		Bool(srvflags.GRPCOnly, false, "Start the node in gRPC query only mode without Tendermint process")
	...
	cmd.Flags().
		Bool(srvflags.JSONRPCEnable, true, "Define if the JSON-RPC server should be enabled")
	cmd.Flags().
	...
	cmd.Flags().
		...
		//nolint:lll
	cmd.Flags().
		Bool(srvflags.JSONRPCEnableIndexer, false, "Enable the custom tx indexer for json-rpc")
	cmd.Flags().
		Bool(srvflags.JSONRPCEnableMetrics, false, "Define if EVM rpc metrics server should be enabled")
...
func startInProcess(
	...
	if config.JSONRPC.EnableIndexer {
		...
		client, ok := clientCtx.Client.(rpcclient.Client)
		if !ok {
			return errors.New("clientCtx.Client not implements rpcclient.Client")
		}
		indexerService := ethermintserver.NewEVMIndexerService(
			idxer,
			client,
		)
	...
	if config.JSONRPC.Enable {
		...
		httpSrv, httpSrvDone, err = ethermintserver.StartJSONRPC(
			ctx,
			clientCtx,
			tmRPCAddr,
			tmEndpoint,
