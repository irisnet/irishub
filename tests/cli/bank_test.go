...
func TestIrisCLIBankSend(t *testing.T) {
	...
	executeWrite(t, fmt.Sprintf("iriscli bank send %v --amount=10iris --to=%s --from=foo --gas=10000 --fee=0.2iris", flags, barAddr), sdk.DefaultKeyPass)
	tests.WaitForNextNBlocksTM(2, port)
	...
	// test memo
	executeWrite(t, fmt.Sprintf("iriscli bank send %v --amount=10iris --to=%s --from=foo --memo 'testmemo' --gas=10000 --fee=0.2iris", flags, barAddr), sdk.DefaultKeyPass)
	tests.WaitForNextNBlocksTM(2, port)
