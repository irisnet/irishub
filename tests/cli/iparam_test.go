...
func TestIrisCLIParameterChangeProposal(t *testing.T) {
	...
	require.Equal(t,  StatusVotingPeriod, proposal1.Status)
	...
	proposal1 = executeGetProposal(t, fmt.Sprintf("iriscli gov query-proposal --proposal-id=1 --output=json %v", flags))
	require.Equal(t, uint64(1), proposal1.ProposalID)
	require.Equal(t,  StatusPassed, proposal1.Status)

	param := executeGetParam(t, fmt.Sprintf("iriscli gov query-params --key=%v --output=json %v ", "Gov/govDepositProcedure", flags))
	require.Equal(t, param,  Param{Key: "Gov/govDepositProcedure", Value: "{\"min_deposit\":[{\"denom\":\"iris-atto\",\"amount\":\"10000000000000000000\"}],\"max_deposit_period\":30000000000}", Op: ""})
}
...
func TestIrisCLIQueryParams(t *testing.T) {
	...
	param := executeGetParam(t, fmt.Sprintf("iriscli gov query-params --key=%v --output=json %v ", "Gov/govDepositProcedure", flags))
	require.Equal(t, param,  Param{Key: "Gov/govDepositProcedure", Value: "{\"min_deposit\":[{\"denom\":\"iris-atto\",\"amount\":\"10000000000000000000\"}],\"max_deposit_period\":60000000000}", Op: ""})
}
