...
func (k Keeper) HandleAddLiquidity(ctx sdk.Context, msg types.MsgAddLiquidity) sdk.Error {
	reservePoolName, err := types.GetReservePoolName(sdk.IrisAtto, msg.MaxToken.Denom)
	if err != nil {
		return err
	}
...
func (k Keeper) HandleRemoveLiquidity(ctx sdk.Context, msg types.MsgRemoveLiquidity) sdk.Error {
	reservePoolName := msg.WithdrawLiquidity.Denom
	minTokenDenom, err := types.GetTokenDenom(reservePoolName)
	if err != nil {
		return err
	}
