...
func TestValidators(t *testing.T) {
	...
	require.Contains(t, resultVals.Validators[0].Address.String(), "fva")
	require.Contains(t, resultVals.Validators[0].PubKey, "fvp")
...
func TestCoinSend(t *testing.T) {
	...
	// query empty
	res, body := Request(t, port, "GET", fmt.Sprintf("/bank/accounts/%s", someFakeAddr), nil)
	...
	require.Equal(t, "iris-atto", mycoins.Denom)
	require.True(t, initialBalance[0].Amount.Sub(sdk.NewIntWithDecimal(1, 18)).GT(mycoins.Amount))
	...
	require.Equal(t, "iris-atto", mycoins.Denom)
	require.Equal(t, sdk.NewIntWithDecimal(1, 18), mycoins.Amount)
}
...
func TestBonding(t *testing.T) {
	name, password, denom := "test", "1234567890", "iris-atto"
	addr, seed := CreateAddr(t, "test", password, GetKeyBase(t))
	...
	acc := getAccount(t, port, addr)
	coins := acc.GetCoins()
	costFee := sdk.NewIntWithDecimal(2, 10).Mul(sdk.NewInt(resultTx.DeliverTx.GasUsed))
	require.Equal(t, sdk.NewIntWithDecimal(40, 18).Sub(costFee), coins.AmountOf(denom))
	...
	tests.WaitForHeight(resultTx.Height+1, port)
	costFee = costFee.Add(sdk.NewIntWithDecimal(2, 10).Mul(sdk.NewInt(resultTx.DeliverTx.GasUsed)))
...
func getAccount(t *testing.T, port string, addr sdk.AccAddress) auth.Account {
	res, body := Request(t, port, "GET", fmt.Sprintf("/bank/accounts/%s", addr), nil)
	require.Equal(t, http.StatusOK, res.StatusCode, body)
	var accInfo bank.BaseAccount
	err := cdc.UnmarshalJSON([]byte(body), &accInfo)
	require.Nil(t, err)
	acc := convertToAuthAcc(t, accInfo)

	return acc
}

func convertToAuthAcc(t *testing.T, accInfo bank.BaseAccount) auth.Account {
	cliCtx := context.NewCLIContext().WithCodec(cdc)
	var coinsString bytes.Buffer
	first := true
	for _, coin := range accInfo.Coins {
		if !first {
			coinsString.WriteString(",")
		}
		coinsString.WriteString(coin)
		first = false
	}
...
func doSend(t *testing.T, port, seed, name, password string, addr sdk.AccAddress) (receiveAddr sdk.AccAddress, resultTx ctypes.ResultBroadcastTxCommit) {
	...
	res, body := Request(t, port, "POST", fmt.Sprintf("/bank/%s/send", receiveAddr), jsonStr)
	require.Equal(t, http.StatusOK, res.StatusCode, body)
...
func doDelegate(t *testing.T, port, seed, name, password string, delegatorAddr, validatorAddr sdk.AccAddress) (resultTx ctypes.ResultBroadcastTxCommit) {
	...
	}`, name, password, accnum, sequence, chainID, delegatorAddr, validatorAddr, "iris"))
